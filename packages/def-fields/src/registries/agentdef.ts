...
export const agentDefRegistry: DefRegistry = {
  ...
  fields: [
    ...
    { key: "skills", label: "Skills", group: "Tools & access", type: "string-list",
      ...
      unsetMeans: "inherits the parent def" },
    { key: "tool_choice", label: "Tool choice", group: "Tools & access", type: "object",
      hint: "Whether, and which, tool the model must call — and for how long. A forcing choice can never apply to every call, or the run could not finish.",
      unsetMeans: "auto — the model decides",
      fields: [
        { key: "mode", label: "Mode", group: "Tools & access", type: "enum", options: ["auto", "none", "required", "tool"],
          hint: "auto lets the model decide; none forbids tool calls; required forces some tool; tool forces the one named below." },
        { key: "name", label: "Tool", group: "Tools & access", type: "text", placeholder: "WebSearch",
          hint: "The tool to call. Required with mode tool, refused with any other mode." },
        { key: "until", label: "Applies until", group: "Tools & access", type: "enum", options: ["first_call", "until_called", "always"],
          hint: "first_call: only the run's first model call (the default). until_called: until the model makes a satisfying call. always: every call — not allowed for required or tool." },
      ] },
    { key: "output_format", label: "Output format", group: "Tools & access", type: "object",
      hint: "A JSON Schema the final answer must follow. Where the model supports it the answer is held to the schema; either way a JSON answer is returned parsed as result.structured.",
      unsetMeans: "free text",
      fields: [
        { key: "name", label: "Name", group: "Tools & access", type: "text", placeholder: "output",
          hint: "A label for the schema — letters, digits, _ and -, up to 64. Defaults to output." },
        { key: "schema", label: "Schema", group: "Tools & access", type: "json",
          hint: "The JSON Schema. Its root must be type object." },
      ] },
    ...
    { key: "recall_attach_traces", label: "Recall searches traces", group: "Memory", type: "bool",
      hint: "Every recall this agent makes ALSO searches the trace index with the same query and returns those turns as a separate block. Needs the trace index enabled AND backfilled — an empty index yields zero turns silently, so check the count the response reports before concluding it is off.",
      unsetMeans: "off", advanced: true },
    { key: "recall_include_turns", label: "Recall attaches turns", group: "Memory", type: "bool",
      hint: "Every recall this agent makes also returns the conversation turn each fact came from. A distilled fact is tenseless; its turn is timestamped. Needs history_scope to permit reading the caller's own chats too — this grants the offer, history_scope grants the read.",
      unsetMeans: "off", advanced: true },
