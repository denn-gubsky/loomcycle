...
Runtime-only across all three phases: **no schema migration, no new wire method**, and the adapters are unchanged — `@loomcycle/client` 1.30.0, Python 1.25.0, `@loomcycle/explorer` 0.4.0 (`GET /v1/_retention` has no adapter twin). Images `denngubsky/loomcycle` + `denngubsky/loomcycle-toolbox` rebuild at 1.32.0. Merged in #796–798.
