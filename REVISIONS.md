...
- **The eval gate (`make memory-eval-mock`).** A deterministic offline harness — mock provider, stub embedder, in-memory SQLite, no network, no API key, ~3 seconds — that pins nine runtime invariants: watermark advance + lease exclusion, idempotent re-consolidation, soft supersession, provenance completeness, total user erasure, hybrid retrieval, the dedup bands, a loudly-refused quota write, and a negative sweep for a planted secret / distractors / facts never stated. The corpus is deliberately adversarial: a pass that "captures everything" fails it. It also runs under plain `go test ./...`, so it gates every PR rather than only the operator who remembers the target.
