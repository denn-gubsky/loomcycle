...
**🧰 The dev sandbox is now a deterministic code-js executor, not an LLM.** The `dev/sandbox` agent's real job was always **mechanism** — translate a caller's commands into `sandbox_*` tool calls and report the results — not judgment. Running that on an LLM bought nothing and cost plenty: token spend, latency, and a flakiness class a smoke test caught in the act on a slow local model — it mis-typed `network:"eggress"` (silently falling back to no network), let the sandbox session get reaped between its slow turns, and finally timed out with `context deadline exceeded`, all before running a single real command. Every failure was mechanism, not reasoning — the same finding that moved `memory/consolidator` to code-js in v1.36.0. So `dev/sandbox` (the agent **and** its tool-driving skill) is **removed**, replaced by a deterministic **`dev/exec`** code-js agent (RFC J).

**`dev/exec` runs a JSON command envelope.** A caller sends `{commands, files, read, network, workspace, keep_open, session_id, continue_on_error, browser}` and the agent runs it deterministically: open (default `network:"egress"`) → write `files` → run `commands` in order, stopping at the first failure unless `continue_on_error` → read `read` artifacts → close, returning `{ok, session_id, results:[{command, ok, output}], artifacts}`. It reuses a warm container by `session_id` (never closing one it did not open, unless `close:true`), runs a best-effort `gh auth setup-git` at open so an injected `sandbox_github` token works, and drives an optional `browser` step list against `mcp__browser__*` (which cleanly gives the headless-browser work a deterministic consumer). Zero-token, deterministic, replay-safe. The judgment it used to fake — read a failure, decide the fix, re-run — moves to the **caller**, which is where it belongs: `dev/exec` executes, and the caller (an LLM parent, via the rewritten `dev/sandbox-usage` skill) inspects `results` and sends a corrected envelope.

**The `sandbox` bundle stays flag-independent (Path B).** A `provider: code-js` agent is boot-fatal without `LOOMCYCLE_CODE_AGENTS_ENABLED`, so putting `dev/exec` in the widely-used `sandbox` bundle would force that flag onto every deployment that just wants the sandbox tools. Instead it ships in a **new opt-in `dev-exec` bundle** (like doc-colorizer/memory, deliberately kept out of default stacks). The `sandbox` bundle keeps only the `mcp_servers.sandbox` wiring (incl. the `$cred:sandbox_github` git-token header) and the rewritten delegation skill — layer it to grant your own agent `mcp__sandbox__*`, add `dev-exec` for the executor. An execution test runs the shipped code-js body through the real agent loop against a fake `mcp__sandbox__*` toolset, so the envelope→tool translation (open/write/exec/read/close order, stop-at-first-failure, reused-session-not-closed, empty-envelope-noop) is asserted on the observed call sequence — not on the source text.

⚠️ **Breaking: `dev/sandbox` is gone.** Add `dev-exec` to `LOOMCYCLE_PRESETS` (alongside `sandbox`) and set `LOOMCYCLE_CODE_AGENTS_ENABLED=1`; spawn **`dev/exec`** with a JSON envelope where you used to spawn `dev/sandbox` with a natural-language prompt (grep `name:"dev/sandbox"` for call sites). No migration, no schema change, no wire change; adapters unchanged (`@loomcycle/client` 1.30.0, `@loomcycle/explorer` 0.5.0, Python 1.25.0), so their publishes skip-but-green.
