...
5. **A2 — `ErrSubscriptionQuotaExhausted` detection.** v0.11.9 exported this sentinel as a stub; v0.11.10 wires the actual detection. In `Driver.Call()`'s event-channel wrapper, an error event whose text contains both "429" and "subscription" (case-insensitive) is prefixed with the sentinel so `errors.Is(err, ErrSubscriptionQuotaExhausted)` matches. Tier-policy `fallback_on_error: true` consumers branch on this specific case to fall back to API-key Anthropic, distinct from generic 429 rate-limiting (which the inner driver retries transparently). Generic 429s pass through unwrapped.
