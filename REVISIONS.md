...
**The test double is why this escaped a suite that executes the shipped body.** The harness returned the child's bare final text — a shape the runtime never produces — so the parser was only ever exercised against a reply that had already been unwrapped. It now prepends the attribution header by default, which puts every existing scenario on the wire shape; against the unfixed body that turns **twelve** tests red rather than one. The new cases pin the live reply and its real `agent_id`, the fenced / prose-wrapped / object-wrapped / bare shapes, and the genuinely-unusable reply (nothing written, watermark held, lease returned, parse failure reported with its raw prefix). One residual gap, stated rather than papered over: the header literal in the double mirrors `formatSubAgentOutput` by hand, and nothing links the two — a change to the runtime's header format would not fail this suite.
