...
- Selecting the bundle now requires **`LOOMCYCLE_CODE_AGENTS_ENABLED=1`**. A `provider: code-js` agent selected without it **fails boot** by design — deliberately, since a consolidator that cannot run should say so at boot rather than idle silently.
- The scheduler decides parallel-vs-serial fan-out by resolving the **scheduled** agent's provider and serialising when it looks local. That agent is now `code-js`, which is never local, so a deployment that used to serialise will now dispatch up to four passes in parallel while their extractor children *do* land on the local box. **Set `LOOMCYCLE_MAX_CONSOLIDATION_CONCURRENCY=1` if your models are local.** Fixing the probe to follow the spawn tree is a separate change.
