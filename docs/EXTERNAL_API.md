...
- **A peer document as a federated source (RFC CE).** Declare a peer under `document_sources:` (see `loomcycle.example.yaml`), then bind a local document to a peer document with `Document op=set_remote` (`source` + `remote_ref`) and pull its keyed chunks with `op=sync`. Sync reconciles by `natural_key`: a new key is created, a diverged body is updated in place (the old body kept in chunk history), and a chunk with no `natural_key` is excluded and counted. Auth + SSRF mirror the remote memory backend (`api_key_env` is a credential-allowlisted env-var *name* resolved at use time; the peer host is dialed through the SSRF guard). This first slice is content-only and PULL-direction (push + full-tree reconciliation are follow-ons).
