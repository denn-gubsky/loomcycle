...
- **A peer document as a federated source (RFC CE).** Declare a peer under `document_sources:` (see `loomcycle.example.yaml`), then bind a local document to a peer document with `Document op=set_remote` (`source` + `remote_ref`) and reconcile its keyed chunks with `op=sync`. `direction:pull` (default) copies the peer's chunks in; `direction:push` writes this document's chunks up to the peer. Either way sync reconciles by `natural_key`, carrying each keyed chunk's **body, tags, hierarchy** (it lands under its keyed parent at its sibling position — a chunk whose parent isn't keyed re-homes to the root) and its **manual cross-reference links**. A new key is created; a diverged body is updated in place (the overwritten body kept in the *losing side's* chunk history — the local document for pull, the peer for push); a chunk with no `natural_key` is excluded and counted; auto `[[name]]` links are regenerated from bodies on each side, so they aren't synced. `op=diff_remote` is the read-only dry-run: it aligns keyed chunks by `natural_key` and reports `only_local` / `only_remote` / `diverged` / `retagged` / `reparented` / `same` (plus the unkeyed and edge-drift counts) without touching either side, so you can see exactly what a pull or push would change first. Auth + SSRF mirror the remote memory backend (`api_key_env` is a credential-allowlisted env-var *name* resolved at use time; the peer host is dialed through the SSRF guard). A source can be declared in `document_sources:` yaml **or authored at runtime** as a versioned, tenant-scoped `DocumentSourceDef` substrate — over HTTP (`POST /v1/_documentsourcedef`), MCP (`documentsourcedef`), gRPC (`DocumentSourceDef`), or the TS/Python adapters (`documentSourceDef` / `document_source_def`); `set_remote`/`sync` resolve a name against the tenant's substrate first, then static yaml, then the shared substrate.
