...
| `History`   | Always registered (RFC BE). Per-agent `history_scope:` YAML gate (default-deny). Browse/search/annotate PAST chats (a chat = a session). 9 ops (`list`/`get`/`search`/`rename`/`annotate`/`pin`/`archive`/`recap`/`resume`); `get format:markdown` exports the transcript; `recap` stores a refreshable LLM summary on the chat; `resume` returns a continuation handle. Owner-scope-aware — `self`/`user`/`tenant`/`global`, with the owner resolved server-side from the run identity and `global` (cross-tenant) admin-only. Every by-id read folds a cross-scope row to an opaque not-found. Per-chat token/cost/run-count stats included; transcripts are already-redacted at write time. Supersedes the removed `Context op=history`. See `docs/HISTORY.md`. |
...
**`History` is reachable off-run via `POST /v1/_history` and the LoomCycle MCP meta-tool `history` (RFC BE).** Same posture as Path/Document — one op-discriminated `Connector` method, owner-scope + tenant resolved server-side from the principal, `ScopeTenant`-gated (admin also satisfies), and the cross-tenant `global` scope admin-only. The gRPC RPC and the TS/Python adapter twins are a later PR. See `docs/HISTORY.md`.
