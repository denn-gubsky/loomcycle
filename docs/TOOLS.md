...
| `Recall`    | Always registered (RFC CT Phase 1), like `Memory`. Per-agent `tools:[Recall]`. One free-text `Recall(query)` op over **two** sources, merged by score, returning the originals verbatim. (1) The spans this run's context distillation EVICTED — the turns behind a compaction cut, what a recap drops, the reasoning + observation a stateful step discards — harvested into a **run-scoped, in-memory, FIFO-capped** embedded index. That index is built only when the agent sets `context.recall` **and** an embedder is configured; otherwise it is nil and the harvest is a no-op, so an unopted run is byte-identical to before the feature. (2) The agent's durable `Memory`, searched across its permitted scopes (user→agent→tenant) as a silent fallback that only ever ADDs hits — which is why the tool is worth granting even with `context.recall` off. Free text rather than identifiers, because a model queries fluently in plain language and barely reproduces ids or its own exact prior wording. Run-scoped on purpose: indexing every evicted turn into the durable per-scope vector store would pollute the agent's memory and add writes to the distillation hot path. Degrades gracefully — index-only, memory-only, or a clean "nothing matched"; a harvest failure is never fatal. NOT wired on the resume path: a resumed run replays past distillations without harvesting, so nothing double-indexes. See `docs/CONFIGURATION.md` for `context.recall`. |
...
**`Path` + `Document` are reachable off-run, on every transport (v1.4.0).** Besides being callable in-band by an agent, both are exposed as direct operations — `POST /v1/_path` + `POST /v1/_document` (HTTP), the `Path` / `Document` gRPC RPCs, the LoomCycle MCP meta-tools `path` / `document`, and `client.path()` / `client.document()` in the TS (`@loomcycle/client`) and Python adapters — so a human or UI can co-author the same scoped namespace + documents agents build, without spawning a run. All four surfaces dispatch through one op-discriminated `Connector` method per tool; **scope (agent/user/tenant) and tenant are resolved server-side from the authenticated principal, never the wire**. Both are tenant-confined (`ScopeTenant`; `substrate:admin` also satisfies). See `docs/PATH.md` and `docs/DOCUMENTS.md`.

**`History` is reachable off-run via `POST /v1/_history`, the LoomCycle MCP meta-tool `history`, the gRPC RPC, and the TS/Python adapter twins (RFC BE).** Same posture as Path/Document — one op-discriminated `Connector` method, owner-scope + tenant resolved server-side from the principal, `ScopeTenant`-gated (admin also satisfies), and the cross-tenant `global` scope admin-only. `related` is a new op on the same dispatch (no wire change). See `docs/HISTORY.md`.
...
**Put the credential in `api_key_env`, never in `base_url`.** A transport failure surfaces the URL in the error text (Go wraps it in a `*url.Error`), and while a `user:password@host` password is redacted there, a token in the query string or in the username is not — it would reach logs verbatim.
...
`Document op=verification_stats` reports how much of a scope is verified — counting CLAIMS only, since the pass writes an entity identity node beside each fact (the subject it is about) and a name can never carry a span; `list_facts` still returns those by default, because document sync reconciles them, so a listing meant for a person to read should pass `claims_only: true` to match the figure; `Document op=verbatim_answer` answers a lookup question with a stored fact quoted next to its span, and refuses — with a reason — whenever the best match is unverified, below the similarity floor, or tied with another fact. `loomcycle memory-eval-judge` scores the judge against a corpus that measures false refusals and false admissions in both directions.
