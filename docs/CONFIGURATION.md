...
**Spawn confinement.** A sub-agent's volume set is the **narrow-only** intersection of (child-declared) ∩ (parent's active bindings); where both hold a volume, the ro/rw flag resolves to the more restrictive of the two. A child can never gain a volume its parent lacks — mirroring host-allowlist narrowing.

**Backward compatibility.** With no `volumes:` block, the legacy `LOOMCYCLE_READ_ROOT` / `WRITE_ROOT` synthesize a `default` volume, so a deployment with no volumes config and no agent bindings is **byte-identical** to before. When `ReadRoot` is a broader parent of `WriteRoot`, a companion read-only `default-read` volume is synthesized alongside the rw `default` (= WriteRoot). To migrate cleanly off the env-var split, declare explicit `volumes:` instead.
