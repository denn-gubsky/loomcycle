...
- **Applied live (this build):** `providers`, `models`, `tiers`, `provider_priority`, `user_tiers`, `agents`, `defaults`. A changed provider `base_url` / `options` (num_ctx, timeouts) / added-or-removed provider takes effect on the **next** run — the provider set + resolver policy are rebuilt in place. `user_tiers` / `agents` / `defaults` are read live off the config, which is now held behind an atomic pointer, so a reload swaps the whole config in one step and the next run's admission/agent-resolution sees the new values. In-flight runs keep the config + provider they already resolved. The primary uses: retuning a local backend (Ollama/vLLM/llama.cpp) and editing a static agent def or tier policy — all without a restart.
- **`restart_required`:** any changed section consumed by a boot-built subsystem — `memory` (embedder / consolidation), `concurrency` (the semaphore + provider gates), `scheduled_runs`, `skills`, `channels`, the host allowlists, the listen address, the store DSN — is reported here, not applied; a restart picks it up. Later releases add these subsystems' reloaders.
