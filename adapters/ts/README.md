...
- **Breaking — `tools` replaces `allowedTools` / `allowed_tools`** — the per-run tool allowlist is now a single canonical key. The `runStreaming` / `continueSession` option is **`tools`** (was `allowedTools`) and the `AgentDefOverlay` + library wire field is **`tools`** (was `allowed_tools`), matching the loomcycle YAML rename and aligning with Claude Code agent frontmatter. There is no dual-key fallback — update call sites to `runStreaming({ …, tools: [...] })`. Requires a loomcycle server carrying the same rename.
