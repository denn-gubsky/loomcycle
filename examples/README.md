...
## Experiment examples (exp1–exp6, exp6.8, exp11)
...
| [`exp11-skybits-documents/`](exp11-skybits-documents/) | Remote MCP (Skybits shared documents, 59 tools over Streamable HTTP) + Memory: an agent creates/edits Skybits docs and records document facts in user-scoped memory so later runs recall them. | ⚠️ needs a Skybits account + connector key + a provider (see its README) |
