...
// A tenant's code hook runs on its own tenant's runs.
func TestDispatcher_ATenantCodeHookRunsOnItsTenantsRuns(t *testing.T) {
	...
	if out := d.RunPre(context.Background(), Identity{Agent: "a", Tenant: "acme"}, ToolCall{ID: "t1", Name: "Read"}); out.Deny == nil || out.Deny.Text != "ran" {
