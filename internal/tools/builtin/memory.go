...
const memoryDescription = `Persistent storage scoped to this agent, the end-user, or the tenant. ` +
	...
	`Key/value operations: get, set, delete, list, incr, search, merge, append_dedupe, bounded_list. ` +
	`Scope is "agent" (this agent's keyspace, shared across users), "user" (this end-user's keyspace, shared across agents) or "tenant" (shared by every user and agent in the tenant, when granted). ` +
	`Values are JSON. Optional TTL is in seconds. ` +
	`Pass embed=true with embed_text on set to enable semantic search; use op=search with query to find rows by similarity. ` +
	`merge / append_dedupe / bounded_list are atomic reducers — use them instead of get-modify-set when concurrent updates are possible. ` +
...
const memoryInputSchema = `{
  ...
  "properties": {
    ...
    "statement":  {"type": "string", "description": "sql_query / sql_exec: ONE SQL statement. sql_query is read-only (SELECT / WITH … SELECT); sql_exec is DDL/DML (CREATE/INSERT/UPDATE/DELETE/etc.). ATTACH, PRAGMA, load_extension, an inline BEGIN/COMMIT (use sql_begin / sql_commit / sql_rollback), and multiple statements are refused."},
