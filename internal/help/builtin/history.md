---
name: history
description: History — browse, search, and annotate PAST chats (a chat = a conversation session). Owner-scoped (self/user/tenant/global), tenant-isolated, with list/get/search/rename/annotate/pin/archive.
---
...
- Continuing a past chat is a `POST /v1/runs` with its `session_id` — History
  reads and annotates chats; it does not itself start runs.
