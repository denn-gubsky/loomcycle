...
its current siblings. **Leave `new_parent_id` empty to move a chunk directly
under the document's title.**

## Arguments

- `id` (required) — the chunk to move.
- `new_parent_id` — the chunk that becomes its parent. It must exist and be in
  the SAME document. Omitted or empty, the chunk moves directly under the
  document's title (its root chunk).
...
Lift a nested chunk back up to be a direct section of the document:

```json
{"op": "move_chunk", "id": "9a3e7c1f2b4d4a6e8c0f1a2b3c4d5e6f", "scope": "agent"}
```
