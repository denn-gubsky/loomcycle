...
// TestMigrate_LegacyTeamDefReadsAsNotOperatorAuthored is the same upgrade case
// on the TEAM plane, and it matters for a reason the agent plane does not have.
//
// A team's authorship gates the widened families inside its NODE PROMPTS — the
// caller segments a walk hands each spawned agent. A legacy row defaulting the
// other way would let every pre-existing team, including ones an agent wrote,
// aim a fetch the runtime performs under its own authority.
func TestMigrate_LegacyTeamDefReadsAsNotOperatorAuthored(t *testing.T) {
	path := filepath.Join(t.TempDir(), "legacy_team_authorship.db")
	ctx := context.Background()

	{
		...
		for _, stmt := range []string{
			...
			`INSERT INTO teamdefs(def_id, name, version, definition, created_at, tenant_id)
			 VALUES ('tdf_legacy', 'legacy-team', 1, '{"entry":"a"}', 1, '')`,
		} {
			if _, err := s.db.ExecContext(ctx, stmt); err != nil {
				t.Fatalf("legacy schema %q: %v", stmt[:40], err)
			}
	...
	row, err := s.TeamDefGet(ctx, "tdf_legacy")
	if err != nil {
		t.Fatalf("read the legacy row after migrating: %v", err)
	}
	if row.OperatorAuthored {
		t.Error("a PRE-MIGRATION team reads as operator-authored — every legacy team, " +
			"including ones an agent wrote, would be able to aim a runtime fetch")
	}
	if row.Name != "legacy-team" {
		t.Errorf("the migration disturbed the row: name = %q", row.Name)
	}

	created, err := s.TeamDefCreate(ctx, store.TeamDefRow{
		DefID: "tdf_new", Name: "new-team", Definition: json.RawMessage(`{"entry":"a"}`),
		OperatorAuthored: true,
	})
	if err != nil {
		t.Fatalf("create after migrating: %v", err)
	}
	back, err := s.TeamDefGet(ctx, created.DefID)
	if err != nil {
		t.Fatalf("read back: %v", err)
	}
	if !back.OperatorAuthored {
		t.Error("a team written after the migration lost its authorship — the column is write-only")
	}
}
