...
const spawnPerRunProps = `
	...
	"hooks": {"type": "object", "description": "Hooks this child adds to its agent's own, by event (pre, post, post_failure, agent_start, agent_stop, subagent_start, subagent_stop, pre_compact, post_compact, run_end). Each event takes a list of entries: a hook definition name (\"gate\", or \"gate@3\" pinned) or an inline webhook {name, url, fail_mode, timeout_ms, headers}. Only ever added — nothing here removes a hook the agent carries — and sub-agents it starts inherit them. They cannot widen the hosts the agent may reach.", "additionalProperties": {"type": "array"}},
	"tool_hooks": {"type": "object", "description": "Hooks for one tool of this child's agent, by tool name: {\"WebFetch\": {\"pre\": [...], \"post\": [...], \"post_failure\": [...]}}. The tool must be one the agent has. Same entries and rules as hooks.", "additionalProperties": {"type": "object"}},
...
func toolDescriptors() []loommcp.ToolDescriptor {
	return []loommcp.ToolDescriptor{
		...
		{
			Name:        "hookdef",
			Description: "Author and version a reusable HOOK \u2014 one gate on an agent: the event it answers (a tool call before or after it runs, or a point in the run such as its start or each finished answer), the tools it matches, its body (a code-js script defining hook(ev), or a webhook URL), its fail mode and timeout. Ops: create, fork, get, list, promote, retire, verify, delete. Definitions are VERSIONED and immutable: create and fork each mint a new version rather than editing one, and promote decides which version is served when a hook is named without a version. Confined to your own tenant: rows are stamped from your token, and another tenant's def reads back as not-found rather than forbidden. A hook definition fires on nothing by itself \u2014 an agent definition, a team definition or a run names it, and that decides which agent it gates. A code-js body is compiled when it is saved, and is refused unless the server enables code hooks. Do NOT use it to change what an agent may do in the first place: that is the agent's own tools and grants (agentdef); a hook narrows or checks calls within them.",
			InputSchema: builtinSchema("hookdef"),
		},
		{
			Name:        "mcpserverdef",
