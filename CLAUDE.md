...
**Currently planned (v0.8.x → v1.0 outline):** LoomCycle MCP (loomcycle exposes itself as an MCP server so external orchestrators can spawn/configure agents — v0.8.x capstone, v0.8.13), Question tool (human-in-the-loop primitive, v0.8.14), Pause / Resume / Snapshot (runtime-wide quiesce + cross-version-portable JSON snapshot; precondition for v0.9.x multi-replica HA, v0.8.15), high-load capacity work (per-tenant fairness, OTEL, run-status cache, heartbeat sweeper hardening — v0.9.x). See `docs/PLAN.md` for the public roadmap and `doc-internal/PLAN.md` for decision history.
