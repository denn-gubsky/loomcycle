...
**Currently planned (v0.8.x → v1.0 outline):** Channel tool (inter-agent message bus, v0.8.2), LoomHelp tool (runtime introspection — every tool's metadata + agent context, v0.8.3), LoomCycle MCP (loomcycle exposes itself as an MCP server so external orchestrators can spawn/configure agents — v0.8.x capstone, v0.8.4), high-load capacity work (per-tenant fairness, OTEL, run-status cache, heartbeat sweeper hardening — v0.9.x). See `docs/PLAN.md` for the public roadmap and `doc-internal/PLAN.md` for decision history.
