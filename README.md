...
| **AgentDef + SkillDef + MCPServerDef substrate** — content-addressed by SHA-256, runtime-mutable, push-at-boot from your container image; verify-or-fork across deployments (runtime stdio MCP defs interpolate `${ENV}` at spawn — F39; AgentDef create/fork round-trips `*_def_scopes` so runtime-authored meta-agents keep their authority — F40) | v0.8.5 / v0.8.22 / v0.9.x |
...
| **Embedded React Web UI** at `/ui` — full manual management console behind a **collapsible left-sidebar nav** (icons-only / icons+labels): Library admin (agents / skills / MCP servers), **Integrations admin** (webhooks / A2A server-cards / A2A agents / memory-backends), **Run launcher** (single run with live SSE transcript + multi-turn continue; **fan-out** of N independent runs; **orchestrator** view of a parallel-spawn parent→child tree), Channels view with **manual publish + broadcast / await fan-out/fan-in**, Memory editor, Activity Monitor, audit log, **Schedules admin** (fork + standalone create + version activate + `max_fires`), and an **interactive terminal** on the Run view (inline interruptions · mid-run steering · persistent sessions) | v0.8.21 → v0.26.0 |
| **Interactive terminal** — drive a live agent like Claude Code: answer Interruption questions **inline**, inject instructions into a running agent **mid-turn** (`POST /v1/runs/{run_id}/input`), and run a **persistent** `interactive` session that parks at end_turn (`awaiting_input`) waiting for your next instruction (Cancel to end). Pairs with the per-agent **`unbounded_iterations`** flag for an uncapped always-on agent; steering routes to the owning replica in a cluster. A persistent run **survives leaving the terminal** and is **re-attachable** (`GET /v1/runs/{run_id}/stream`, "resume in terminal"); tool results collapse by default | v0.26.0 → v0.27.0 |
...
**v0.26.2 — Patch: runtime-authored meta-agents keep their capability gates (F40).** A meta-agent that forks / promotes / schedules other agents needs its `*_def_scopes` (`agent_def_scopes` / `skill_def_scopes` / `schedule_def_scopes` / `a2a_server_card_def_scopes` / `a2a_agent_def_scopes`) to survive a reload, but the `AgentDef` create / fork overlay dropped all five — they had no field in the persisted overlay shape (`mergedDef`) or the read adapter (`lookup.SubstrateAgentDef`) — so the agent came back default-deny and could author nothing. The five now round-trip across the same chain `channels` / `evaluation_scopes` / `interruption` use (the substrate-def slice of the F14 closure), deliberately excluded from `content_sha256` (authority, not content — existing rows stay byte-stable). `agent_def_scopes` + `skill_def_scopes` are consumed in-loop today (this directly unblocks the meta-agent); `schedule` / `a2a` def-scopes round-trip for parity. Runtime-only; no `@loomcycle/client` bump.

**v0.26.1 — Mid-run steering goes multi-replica.** v0.26.0's steering was single-replica (a `POST /v1/runs/{run_id}/input` had to hit the replica running the run). v0.26.1 adds a `coord.SteerCoordinator` (the twin of the cancel coordinator): a steer that misses the local registry routes over the backplane to the owning replica (by `runs.replica_id`), awaits an ack, and reports delivered — the owning side dispatching to its local registry only (no re-broadcast storm). Single-replica is byte-identical (nil coordinator → 404 as before). Ships with an end-to-end regression test driving the full interactive flow (start → park → `/input` → `steer` → resume) through the real server. Runtime-only; no `@loomcycle/client` bump.

**v0.26.0 — The interactive terminal: drive an agent live from the Web UI, Claude-Code-style.** The `/run` terminal becomes interactive across three additions. **(1) Inline interruptions** — an agent's Interruption question renders in the live terminal (option buttons / free-text) and resolves on the same open SSE stream, no trip to the `/interrupts` inbox. **(2) Mid-run steering** — `POST /v1/runs/{run_id}/input` injects an unsolicited instruction into an in-flight run; a per-run queue (`internal/steer`) is drained at the top of each loop iteration and appended as a user turn before the next model call (never splitting a tool_use/tool_result pair), surfaced live by a new `steer` SSE event and persisted as a `user_input` transcript row for replay; bearer + tenant-gated, single-replica. **(3) Persistent interactive runs** — `interactive: true` makes the loop **park at end_turn** (emitting `awaiting_input`) waiting for the next instruction, resuming on it and ending only on Cancel; pair it with the new per-agent **`unbounded_iterations`** flag (lifts the `max_iterations` cap for LLM agents, 1<<20 backstop) for a true always-on terminal agent. The `/run` pane gains an always-on prompt (steer while running, continue between turns) + an interactive-session toggle. **Wire addition** (additive): one route + two SSE events + two run flags; **no `@loomcycle/client` bump** (the SPA ships its own client; a steering twin for external orchestrators is deferred).

**v0.25.3 — Patch: dynamic stdio MCP server `${ENV}` is interpolated (F39).** Runtime-only — no wire change, no `@loomcycle/client` bump. A runtime-created **stdio** MCP server (`POST /v1/_mcpserverdef`) whose `env` used `${LOOMCYCLE_*}` got the **literal** placeholder in the child (and, since def env is appended after the inherited environ, the literal also clobbered the real inherited var) — a dynamic Telegram MCP hit `/bot${LOOMCYCLE_TELEGRAM_BOT_TOKEN}/sendMessage` → HTTP 404, while the identical *static* yaml server worked. Cause: config-load `ExpandEnv`s the whole yaml so static defs arrive pre-expanded, but a dynamic def stores raw `${...}` (kept literal per F32) and `spawnStdio` emitted env without expansion. Fix: `spawnStdio` now `ExpandEnv`s command/args/env values **at spawn** — idempotent for static, corrective for dynamic, and not baked into the stored def (the placeholder, not the secret, persists). A runtime-authored stdio MCP with `${LOOMCYCLE_*}` env now works like the static one.
