...
| **Interactive terminal** — drive a live agent like Claude Code: answer Interruption questions **inline**, inject instructions into a running agent **mid-turn** (`POST /v1/runs/{run_id}/input` → a `steer` SSE event), and run a **persistent** `interactive` session that parks at end_turn (`awaiting_input`) waiting for your next instruction (Cancel to end). Pairs with the per-agent **`unbounded_iterations`** flag for an uncapped always-on agent; steering routes to the owning replica in a cluster | v0.26.0 → v0.26.1 |
