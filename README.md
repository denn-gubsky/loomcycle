...
- [`docs/SKYBITS.md`](docs/SKYBITS.md). Skybits shared document workspace via MCP: the `mcp_servers.skybits` block, connector-key vs per-user OAuth token bindings, tool allowlist + agent opt-in, and the Memory interplay (explicit fact-banking; Skybits content stays outside the `doc.chunk:` keyspace).
