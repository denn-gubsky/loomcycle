...
| **Built-in tools** | Read / Write / Edit / Grep / Glob / NotebookEdit / HTTP / WebFetch / WebSearch / Bash / Agent / Skill / Memory / Recall / Channel / History / Path / Document / AgentDef / SkillDef / Evaluation / Interruption / Context. |
| **Memory** | Four facets on one tool: key-value, **vector** (sqlite-vec / pgvector), **SQL** (a per-scope database, sqlite + postgres tiers), and an agentic **memory layer** (background consolidation, hybrid recall, bi-temporal entity graph with a layered ontology). Every recalled row is labelled with its class — a `fact` a consolidator distilled, a `note` an agent wrote, or `document` prose — so an agent can tell what it was told from what it merely jotted down. Rows also carry a time axis — when a thing was *said* (`observed_at`) and when it was *true* (`valid_at`/`invalid_at`) — so a `when` window or an `as_of` query can ask about a moment, not just a topic. Scoped agent / user / tenant / run, tenant-isolated, capability-gated — and an entity type in the tenant ontology may **declare which scope its facts belong in**, so organisation knowledge lands where everyone reads it instead of in whoever happened to mention it (operator config, not per-fact inference; every uncertainty declines to move the fact). |
| **Documents & paths** | Chunked-graph **Documents** (chunk bodies in Memory, structure in SQL Memory), **semantically searchable** — bodies embed on write, with per-type policy for prose, Mermaid labels and image captions + vision descriptions. Inline `[[name]]` links become graph edges, `![[…]]` embeds transclude, plus backlinks / related / unlinked-mentions discovery, a per-chunk body-history with diff, queryable tags and type/status, images and Mermaid diagrams, and Markdown + JSON Canvas import/export — all addressed through a Unix-like **Path** VFS. |
...
| **Multi-agent** | Sub-agents via the `Agent` tool, `parallel_spawn` fan-out, external batch fan-out, resident/interactive sub-agents, a structured **state hand-off** from a stateful child to its parent, and a state-machine **Team** orchestrator. |
...
| **Multi-tenancy** | Per-principal bearer tokens bound to an authoritative `(tenant, subject, scopes)`; per-route HTTP and per-RPC gRPC gates; tenant isolation across both the state and definition planes. Per-tenant credentials **plus per-user credential self-service** (every user manages its own `scope=user` API tokens — a personal Slack/Telegram bot token — from any transport), per-scope usage/cost attribution, token budgets, data-retention + subject-erasure surfaces, and a derived **directory** (who is here, what do we hold for them). Single-tenant stays the default. |
| **Context** | A compaction subsystem (manual / auto / self, per-agent policy that flows down the spawn tree) and **layered execution context** — instead of one growing transcript, a run can retain a reasoning **recap** or a validated **structured state** Σ, with `mode: auto` picking per resolved model (schema-free recap on a local backend, structured state on a frontier API). A stateful sub-agent hands its Σ up to a fan-out parent, so N children return N compact results rather than N transcripts; an agent may *propose* its state schema, and only an operator adopts it. Every retention mode discards work by design, so an opt-in `context.recall` harvests each **evicted span** into a run-scoped embedded index and gives the agent a free-text **`Recall`** tool over it, falling back to durable memory — the dropped turns stay reachable without being stored forever. Plus context-transform plugins including a secret **redactor**, and `{{memory:…}}` / `{{tool:…}}` prompt expansion. |
| **Operations** | Pause / Resume / Snapshot with cross-instance resume, multi-replica HA (Redis cancel pubsub, DB-backed session locks, singleton sweepers), OTEL + Prometheus, per-tenant fairness, tool-use hooks, and **runtime config reload** (`POST /v1/_config/reload` — validate-before-apply, in-flight runs undisturbed). |
