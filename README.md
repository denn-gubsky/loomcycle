...
- **SQL Memory (RFC AA).** A second facet of the Memory primitive: per-scope SQL databases the runtime owns, isolated from the main loomcycle store, that sandboxed agents query with arbitrary SQL. Memory ops `sql_query` / `sql_exec` (+ `sql_begin` / `sql_commit` / `sql_rollback` for atomic multi-step transactions), durable per-`(tenant, scope, scope_id)` plus an ephemeral per-run scratch DB, default-deny `sql_scopes` ACL, statement timeouts, byte quotas, full audit. sqlite tier (one file per scope, statement-allowlist hardened) for compact installs; **postgres tier (one schema per scope in a separate aux DB, isolated by a per-scope least-privilege role)** for multi-replica — SQL Memory follows the main store backend. On postgres, **vector columns** (pgvector) let agents keep embeddings in their own tables and do semantic KNN + structured filters in one query, with server-side embedding via an `{"$embed": …}` bind directive (no raw vector in context). The "agents that need structured tables today need `Bash + sqlite3`" gap.
