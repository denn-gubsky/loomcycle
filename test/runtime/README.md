...
| `memory-core/` | **Deterministic (mock, sqlite — no embedder).** Built-in Memory non-vector half: admin K/V CRUD (PUT/GET/list/DELETE→404, user + agent scopes) + MemoryBackendDef CRUD including the trust-boundary rejections (empty `shared_key_with_prefix` prefix_pattern) and the closed-enum refusal of the removed external `mem9` kind. |
